...
class FeatureCorrelationBuilder(nn.Module):
    """Build a correlation lookup object after bidirectional feature mixing."""

    def __init__(self, args):
        super().__init__()
        self.args = args
        self.feature_mixer = AlternatingFeatureMixer(args)

    def forward(self, fmap1, fmap2, inp):
        fmap1, fmap2 = self.feature_mixer(fmap1, fmap2)
        corr_fn = CorrBlock(fmap1, fmap2, num_levels=self.args.corr_levels, radius=self.args.corr_radius)
        return corr_fn


class AlternatingFeatureMixer(nn.Module):
    """Mix paired feature maps with local self-attention and global cross-attention."""
    ...
    def __init__(self, args):
        ...
        for n in range(self.num_layers):
            if n == self.num_layers - 1:
                self.blocks.append(
                    FeatureExchangeStage(num_layer=num_layers[n], dim=dim, mlp_scale=mlp_scale, window_size=window_size, cross=False))
            else:
                self.blocks.append(
                    FeatureExchangeStage(num_layer=num_layers[n], dim=dim, mlp_scale=mlp_scale, window_size=window_size, cross=True))
...
class CrossImageAttentionBlock(nn.Module):
...
class FeatureExchangeStage(nn.Module):
    def __init__(self, num_layer, dim, mlp_scale, window_size, cross=False):
        ...
        for n in range(num_layer):
            ...
            self.blocks.append(
                ShiftedWindowSelfAttentionBlock(
                    dim=dim,
                    mlp_scale=mlp_scale,
        ...
        if cross:
            self.cross_transformer = CrossImageAttentionBlock(dim=dim, mlp_scale=mlp_scale, norm='layer')
...
class ConvexUpsampleMask8x(nn.Module):
...
class ConvexUpsampleMask4x(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.up_sample_mask = nn.Sequential(
            nn.Conv2d(in_channels=dim, out_channels=256, kernel_size=3, padding=1, stride=1),
...
class DepthwiseResidualMLP(nn.Module):
...
class MotionEvidenceEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.convc1 = DepthwiseResidualMLP(cor_planes, 256, k_conv=args.k_conv)
        self.convc2 = DepthwiseResidualMLP(256, 192, k_conv=args.k_conv)

        self.convf1 = nn.Conv2d(2, 128, 1, 1, 0)
        self.convf2 = DepthwiseResidualMLP(128, 64, k_conv=args.k_conv)

        self.conv = DepthwiseResidualMLP(64+192, 128-2, k_conv=args.k_conv)
...
class AttentionMotionUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dim):
        super().__init__()
        self.args = args
        self.encoder = MotionEvidenceEncoder(args)
        self.gru = DepthwiseResidualMLP(128+hidden_dim+hidden_dim+128, 128, k_conv=args.PCUpdater_conv)
        self.flow_head = DepthwiseResidualMLP(128, 2, k_conv=args.k_conv)
        ...
        self.aggregator = ContextAttentionAggregator(self.args, 128, heads=getattr(self.args, 'num_heads', 1))
...
class ContextAttentionAggregator(nn.Module):
    ...
    def forward(self, *inputs):
        if len(inputs) == 2:
            attention, feat_mo = inputs
            b, c, h, w = feat_mo.shape
            if attention.dim() == 4:
                attention = attention.reshape(b, h * w, h * w)
            feat_v = self.to_v(feat_mo).view(b, c, h*w)
            feat_o = torch.einsum('b n m, b c m -> b c n', attention, feat_v).contiguous().view(b, c, h, w)
            return feat_mo + feat_o * self.gamma
...
class MotionRefinementBlock(nn.Module):
    def __init__(self, args, hidden_dim):
        ...
        self.encoder = MotionEvidenceEncoder(args)
        self.gru = DepthwiseResidualMLP(128+hidden_dim+hidden_dim+128, d_dim, k_conv=args.PCUpdater_conv)  
        self.flow_head = DepthwiseResidualMLP(d_dim, 2, k_conv=args.k_conv)
        self.aggregator = ContextAttentionAggregator(self.args, d_dim)
...
class TimeConditionedResidualAdapter(nn.Module):
...
class DiffusionMotionUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dim):
        super().__init__()
        self.args = args
        chnn = hidden_dim
        self.conv_ee = TimeConditionedResidualAdapter(chnn, chnn)
...
TransformerModule = FeatureCorrelationBuilder
TransBlocks = AlternatingFeatureMixer
SelfTransformerBlcok = ShiftedWindowSelfAttentionBlock
CrossTransformerBlcok = CrossImageAttentionBlock
BasicLayer = FeatureExchangeStage
UpSampleMask8 = ConvexUpsampleMask8x
UpSampleMask4 = ConvexUpsampleMask4x
PCBlock4_Deep_nopool_res = DepthwiseResidualMLP
SKMotionEncoder6_Deep_nopool_res = MotionEvidenceEncoder
SKUpdateBlock6_Deep_nopoolres_AllDecoder = AttentionMotionUpdateBlock
Aggregator = ContextAttentionAggregator
SKUpdate = MotionRefinementBlock
ConvEE = TimeConditionedResidualAdapter
SKUpdateDFM = DiffusionMotionUpdateBlock
